 ...
 */
public class PrijenosnaCostFunction implements IHFunction {
    ...
    @Override
    public double calculateValueIn(RealVector constantsVector) {

        double[] constants = constantsVector.toArray();
        double[] grad = new double[variables.getRowDimension()];
    ...
    public RealVector calculateGradientValueIn(RealVector constantsVector) {
		...
		for (int i = 0; i < variables.getRowDimension(); i++) {
            ...
            double valueInPoint = calculateValueIn(constantsVector);
    ...
    @Override
    public RealMatrix calculateHesseMatrixIn(RealVector coef) {
        // NOT IMPLEMENTED !
        throw new IllegalStateException("I'm sorry but this method is not" +
                " implemented. Did not find an viable solution to implement the" +
                " matrix besides hardcoding an enormous equation.");
    }
}
